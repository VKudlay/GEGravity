...
*
* 0.1. (new equilibrium in changes):      Y_i * w_i_hat =  A_i_hat * w_i_hat^-theta * sum_j { pi_ij * e^beta * E_j' / P_j_hat^-theta }     (eq 3)
*
*                                         where: P_j_hat^-theta = sum_i { pi_ij * e^beta * A_i_hat * w_i_hat^-theta }                      (eq 4)
*                                                
...
mata: 
void ge_solver(string scalar trade, string scalar partials, string scalar T_hat, string scalar elasticity, string scalar mult_opt,
...
{ 
  ...
  do {
	...
	pi_new = (pi :* B) :* ((A_hat:*(w_hat:^-theta))#J(1,N,1))  :/ (P_hat'#J(N,1,1))
