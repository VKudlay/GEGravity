...
The Stata equivalent of this package uses the ppmlhdfe command created by Correia, Guimarães, & Zylkin (2019) to achieve this. In our example, we will use the [alpaca library](https://cran.r-project.org/web/packages/alpaca/index.html) by Amrei Stammann to do the same via the `feglm` (Fixed-Effect Generalized Linear Model) function.
...
Before proceeding, some further pre-processing is needed. Specifically, the function needs to get a series
...
- The change in welfare $\hat{W}_{j}$ for all importers.

- The general equilibrium trade impact (GETI) $\hat{X}_{ij}$ between all importers and exporters.
